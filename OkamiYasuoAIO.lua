...
local OkamiHeroCount = Game.HeroCount()
local OkamiTimer = Game.Timer
...
local myCounter = 1
local OkamiCSCount = Game.MinionCount
local OkamiCS = Game.Minion
local LocalGameTurretCount 	= Game.TurretCount;
local LocalGameTurret = Game.Turret;
local SpellIsReady = Game.CanUseSpell
local CancastSpell = Control.CastSpell
local _EnemyHeroes
local TotalHeroes
...
GetTarget = function(range)
	...
	elseif OkamiOrb == 2 and OkamiSDKSelector then
		if Yasuo.ap > Yasuo.totalDamage then
			return OkamiSDKSelector:GetTarget(range, OkamiSDKMagicDamage)
		else
			return OkamiSDKSelector:GetTarget(range, OkamiSDKPhysicalDamage)
        end
    elseif _G.GOS then
		if Yasuo.ap > Yasuo.totalDamage then
...
minionCollision = function(target, me, position)
    ...
    for i = OkamiCSCount(), 1, -1 do 
        local minion = OkamiCS(i)
...
GetRecallingData = function(unit)
    for i = 0, unit.buffCount do
        ...
        if buff and buff.name == 'recall' and buff.duration > 0 then
            return true, OkamiTimer() - buff.startTime
        end
    end
    return false
end
...
PredictReactionTime = function(unit, minimumReactionTime)
    ...
    if unit.activeSpell and unit.activeSpell.valid then
        local windupRemaining = unit.activeSpell.startTime + unit.activeSpell.windup - OkamiTimer()
...
UpdateMovementHistory =
    ...
    for i = 1, TotalHeroes do
        ...
        if not _movementHistory[unit.charName] then
            ...
            _movementHistory[unit.charName]['ChangedAt'] = OkamiTimer()
        end
    ...
    for i = 1, TotalHeroes do 
        ...
        if SpellData[unit.charName] then
            for x, v in pairs(SpellData[unit.charName]) do
                if v then
    Okami.Wset.UseW:MenuElement({id = x, name = "Use W on: "..v['name'], value = true})
                end
            end
        end
    end
...
if Game.Timer() > Okami.Rate.champion:Value() and #_EnemyHeroes == 0 then
    for i = 1, TotalHeroes do 
        local unit = _EnemyHeroes[i]
        if SpellData[unit.charName] then
            for x, v in pairs(SpellData[unit.charName]) do
...
if  OkamiOrb == 1 then
   ...
   Okamiharass = mode == 2
   OkamilastHit = mode == 3
   OkamilaneClear = mode == 4
   OkamijungleClear = mode == 4

   Okamicanmove = EOW:CanMove()
   Okamicanattack = EOW:CanAttack()
elseif  OkamiOrb == 2 then
    OkamiSDK = SDK.Orbwalker
    OkamiSDKCombo = SDK.ORBWALKER_MODE_COMBO
    OkamiSDKHarass = SDK.ORBWALKER_MODE_HARASS
    OkamiSDKJungleClear = SDK.ORBWALKER_MODE_JUNGLECLEAR
    OkamiSDKJungleClear = SDK.ORBWALKER_MODE_JUNGLECLEAR
    OkamiSDKLaneClear = SDK.ORBWALKER_MODE_LANECLEAR
    OkamiSDKLastHit = SDK.ORBWALKER_MODE_LASTHIT
    OkamiSDKFlee = SDK.ORBWALKER_MODE_FLEE
    OkamiSDKSelector = SDK.TargetSelector
    OkamiSDKMagicDamage = _G.SDK.DAMAGE_TYPE_MAGICAL
    OkamiSDKPhysicalDamage = _G.SDK.DAMAGE_TYPE_PHYSICAL
elseif  OkamiOrb == 3 then
...
DisableMovement = function(bool)
   ...
   if OkamiOrb == 2 then
       OkamiSDK:SetMovement(not bool)
   elseif OkamiOrb == 1 then
...
GetOrbMode = function()
   if OkamiOrb == 1 then
       if Okamicombo == 1 then
           return 'Combo'
       elseif Okamiharass == 2 then
           return 'Harass'
       elseif OkamilastHit == 3 then
           return 'Lasthit'
       elseif OkamilaneClear == 4 then
           return 'Clear'
       end
   elseif OkamiOrb == 2 then
       OkamiSDKModes = SDK.Orbwalker.Modes
       if OkamiSDKModes[OkamiSDKCombo] then
           return 'Combo'
       elseif OkamiSDKModes[OkamiSDKHarass] then
           return 'Harass'
       elseif OkamiSDKModes[OkamiSDKLaneClear] or OkamiSDKModes[OkamiSDKJungleClear] then
           return 'Clear'
       elseif OkamiSDKModes[OkamiSDKLastHit] then
           return 'Lasthit'
       elseif OkamiSDKModes[OkamiSDKFlee] then
           return 'Flee'
       end
   elseif OkamiOrb == 3 then
       return GOS:GetMode()
   elseif OkamiOrb == 4 then
...
LocalCallbackAdd("Draw", function()
    if Okami.Drawings.Q.Enabled:Value() then 
        Draw.Circle(myHero.pos, 400, 0, Okami.Drawings.Q.Color:Value())
    end

    if Okami.Drawings.E.Enabled:Value() then 
        Draw.Circle(myHero.pos, 475, 0, Okami.Drawings.E.Color:Value())
    end

    if Okami.Drawings.R.Enabled:Value() then 
        Draw.Circle(myHero.pos, 1400, 0, Okami.Drawings.R.Color:Value())
    end
...
LocalCallbackAdd("Tick", function()
    ...
    if GetOrbMode() == 'Lasthit' and Okami.Clear.UseQ:Value() then
        LastHit()
    end
...
function CalcPhysicalDamage(source, target, amount)
  ...
  PassivePercentMod = function(source, target, amount, damageType)
    ...
    SIGroup = function(target)
        ...
        if target then
            if bg and Okami.items.bg:Value() and myHero:GetSpellData(bg).currentCd == 0  and myHero.pos:DistanceTo(target.pos) < 550 then
                Control.CastSpell(HKITEM[bg], target.pos)
            end
            
            
            local tmt = items[3077] or items[3748] or items[3074]
            if tmt and Okami.items.tm:Value() and myHero:GetSpellData(tmt).currentCd == 0  and myHero.pos:DistanceTo(target.pos) < 400 and myHero.attackData.state == 2 then
                Control.CastSpell(HKITEM[tmt], target.pos)
            end
    
            local YG = items[3142]
            if YG and Okami.items.yg:Value() and myHero:GetSpellData(YG).currentCd == 0  and myHero.pos:DistanceTo(target.pos) < 1575 then
                Control.CastSpell(HKITEM[YG])
            end
            
            
            if ignitecast and igniteslot and Okami.items.ig:Value() then
...
AutoR = function()
    ...
    local target = GetTarget(1400)
        --forloop
        if Okami.Rset.UseRA:Value() and Game.CanUseSpell(3) == 0 and number >= Okami.Rset.RCount:Value() and not UnderTurret(DashEndPos(target)) and Okami.Combo.UseR:Value() then
            CastSpell(HK_R, p)
        else 
            if number == 1 and target and Okami.Rset.UseR:Value() and GetDamage(target) > (target.health + target.shieldAD + target.shieldAP) and Game.CanUseSpell(3) == 0 then
    ...
    for i = 1, Game.MissileCount() do
        ...
        if obj then
            ...
            if  target and table.contains(walllist, obj.missileData.name:lower()) and  Game.CanUseSpell(1) == 0 and isOnSegment and (GetDistanceSqr(castp, linesegment) <= 300  * 300)then --
                ...
                if Okami.Wset.UseW[obj.missileData.name:lower()]:Value() then
...
CastE = function(target)
    if Okami.Combo.UseE:Value() and target.pos:DistanceTo() <= 475 and target.pos:DistanceTo() >= 150 and Game.CanUseSpell(2) == 0 and GotBuff(target, "YasuoDashWrapper") == 0 and not UnderTurret(DashEndPos(target)) then  
        Control.CastSpell(HK_E, target)
    else
        if Okami.Combo.UseE:Value() and GotBuff(target, "YasuoDashWrapper") == 0 and target.pos:DistanceTo() <= 475 and target.pos:DistanceTo() >= 150 and GetDamage(target) > (target.health + target.shieldAD + target.shieldAP) and Game.CanUseSpell(2) == 0  then
            Control.CastSpell(HK_E, target)
        end
...
CastR = function(target) 
        ...
        --forloop
        if Game.CanUseSpell(3) == 0 and number >= Okami.Rset.RCount:Value() and not UnderTurret(DashEndPos(target)) and Okami.Combo.UseR:Value() then
            CastSpell(HK_R, p)
        else 
            if  number == 1 and Okami.Combo.UseR:Value() and GetDamage(target) > (target.health + target.shieldAD + target.shieldAP) and Game.CanUseSpell(3) == 0 then
                CastSpell(HK_R, target)
            end 
        end
end
Combo =  function()
    ...
    if target and validTarget(target) then
        SIGroup(target)
        if Okami.Combo.UseQ:Value() then
        CastQ(target)
        end 
        CastR(target)
        CastE(target)
...
Harass = function()
    local target = GetTarget(1200)
    if target and Okami.Harass.UseQ:Value() then
        CastQ(target)
    end
...
LaneClear = function() 
    for i = 1, OkamiCSCount() do 
        ...
        if Game.CanUseSpell(0) == 0 and Okami.Clear.UseQ:Value() then
        ...
        if Game.CanUseSpell(2) == 0 and minion and Okami.Clear.UseE:Value() then
...
LastHit = function()
    for i = 1, OkamiCSCount() do 
        local minion = OkamiCS(i)
        if not minion.dead and minion.visible and minion.isTargetable and minion.isEnemy then
        if Game.CanUseSpell(0) == 0 and Okami.Lasthit.UseQ:Value() then
            local QDmg =  CalcPhysicalDamage(myHero,minion, (myHero:GetSpellData(_Q).level* 25 - 5) + (myHero.totalDamage)  + myHero.totalDamage)
...
Okami_Menu = 
function()
	Okami = MenuElement({type = MENU, id = "Yasuo", name = "YasuoAIO", icon = AIOIcon})
	MenuElement({ id = "blank", type = SPACE ,name = "Dev: Okami"})
	--Combo
	Okami:MenuElement({id = "Combo", name = "Combo", type = MENU})
    Okami.Combo:MenuElement({id = "UseQ", name = "Q", value = true})
	Okami.Combo:MenuElement({id = "UseE", name = "E", value = true})
    Okami.Combo:MenuElement({id = "UseR", name = "R", value = true})

	Okami:MenuElement({id = "Harass", name = "Harass", type = MENU})
	Okami.Harass:MenuElement({id = "UseQ", name = "Q", value = true})

    
	Okami:MenuElement({id = "Clear", name = "Clear", type = MENU})
    Okami.Clear:MenuElement({id = "UseQ", name = "Q", value = true})
    Okami.Clear:MenuElement({id = "UseE", name = "E", value = true})
    

    Okami:MenuElement({id = "Lasthit", name = "Lasthit", type = MENU})
    Okami.Lasthit:MenuElement({id = "UseQ", name = "Q", value = true})
    
    Okami:MenuElement({id = "Rate", name = "Recache Rate", type = MENU})
	Okami.Rate:MenuElement({id = "champion", name = "Value", value = 30, min = 1, max = 120, step = 1})
    ...
    Okami:MenuElement({id = "Qset", name = "Q Settings", type = MENU})
    Okami.Qset:MenuElement({id = "UseQ", name = "AutoQ", value = true})

	Okami:MenuElement({id = "Rset", name = "R Settings", type = MENU})
    Okami.Rset:MenuElement({id = "RCount", name = "Use R on X targets", value = 3, min = 1, max = 5, step = 1})
    Okami.Rset:MenuElement({id = "UseR", name = "KS with R", value = true})
    Okami.Rset:MenuElement({id = "UseRA", name = "Auto R X Enemies", value = true})
    
    Okami:MenuElement({id = "Escape", name = "Flee", type = MENU})
    Okami.Escape:MenuElement({id = "UseE", name = "E", value = true})

    Okami:MenuElement({id = "Wset", name = "WindWall Settings", type = MENU})
    Okami.Wset:MenuElement({id = "UseW", name = "Wind Wall Spells", value = false, type = MENU})
    for i = 1, TotalHeroes do 
        local unit = _EnemyHeroes[i]
        if SpellData[unit.charName] then
            for x, v in pairs(SpellData[unit.charName]) do
    ...
    Okami:MenuElement({id = "items", name = "UseItems", type = MENU})
	Okami.items:MenuElement({id = "bg", name = "Use Cutlass/Botrk", value = true})
	Okami.items:MenuElement({id = "tm", name = "Tiamat/Titcan/Ravenous", value = true})
	Okami.items:MenuElement({id = "yg", name = "Yoomus GhostBlade", value = true})
	Okami.items:MenuElement({id = "ig", name = "Ignite", value = true})

    --Okami.QToggle:MenuElement({id = "recallRocket", name = "Ult on Recall[Beta]", value = true})
    


    Okami:MenuElement({id = "Drawings", name = "Drawings", type = MENU})
    Okami.Drawings:MenuElement({id = "Q", name = "Draw Q range", type = MENU})
    Okami.Drawings.Q:MenuElement({id = "Enabled", name = "Enabled", value = true})       
    Okami.Drawings.Q:MenuElement({id = "Width", name = "Width", value = 1, min = 1, max = 5, step = 1})
    Okami.Drawings.Q:MenuElement({id = "Color", name = "Color", color = Draw.Color(200, 255, 255, 255)})
        --E

    Okami.Drawings:MenuElement({id = "E", name = "Draw Real E range", type = MENU})
    Okami.Drawings.E:MenuElement({id = "Enabled", name = "Enabled", value = false})       
    Okami.Drawings.E:MenuElement({id = "Width", name = "Width", value = 1, min = 1, max = 5, step = 1})
    Okami.Drawings.E:MenuElement({id = "Color", name = "Color", color = Draw.Color(200, 255, 255, 255)})	
	
	
    Okami.Drawings:MenuElement({id = "R", name = "Draw R range", type = MENU})
    Okami.Drawings.R:MenuElement({id = "Enabled", name = "Enabled", value = true})       
    Okami.Drawings.R:MenuElement({id = "Width", name = "Width", value = 1, min = 1, max = 5, step = 1})
    Okami.Drawings.R:MenuElement({id = "Color", name = "Color", color = Draw.Color(200, 255, 255, 255)})	
